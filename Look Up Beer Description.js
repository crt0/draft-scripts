(() => { // anonymous function prevents variable conflicts with other actions
    ...
    if (!beer_list) {
        context.fail('Beer list draft not found. Make sure you customize this ',
    ...
    let mm = MultiMarkdown.create();
    if (!app.htmlToClipboard(mm.render('**' + chosen_style + ':** '
                                       + styles.get(chosen_style))))
